...
/*
 * We return EDOM in places glibc would return ERANGE
 * We can't compatibly change that.  Fake it.
 */
#define DIFFERING_DOM_ERROR EDOM
...
#ifdef TEST_DOUBLE
  check_float ("cos (0.80190127184058835) == 0.69534156199418473",  FUNC(cos) (0.80190127184058835), 0.69534156199418473, DELTA649, 0, 0);
#endif
...
expm1_test (void)
{
  ...
  check_int ("errno for expm1(large) == DIFFERING_DOM_ERROR", errno, DIFFERING_DOM_ERROR, 0, 0, 0);
...
fpclassify_test (void)
{
  ...
  check_int ("fpclassify (1000.0) == FP_NORMAL", fpclassify (1000.0), FP_NORMAL, 0, 0, 0);
...
jn_test (void)
{
  ...
  check_float ("jn (3, -1.0) == -0.0195633539826684059189053216217515083",  FUNC(jn) (3, -1.0), -0.0195633539826684059189053216217515083L, DELTA1287, 0, 0);
  ...
  check_float ("jn (3, 1.0) == 0.0195633539826684059189053216217515083",  FUNC(jn) (3, 1.0), 0.0195633539826684059189053216217515083L, DELTA1291, 0, 0);
...
lgamma_test (void)
{
  ...
  check_int ("errno for lgamma(-integer) == DIFFERING_DOM_ERROR", errno, DIFFERING_DOM_ERROR, 0, 0, 0);
  signgam = 0;
...
pow_test (void)
{
  ...
  check_int ("errno for pow(0,-odd) == DIFFERING_DOM_ERROR", errno, DIFFERING_DOM_ERROR, 0, 0, 0);
  errno = 0;
  ...
  check_int ("errno for pow(0,-even) == DIFFERING_DOM_ERROR", errno, DIFFERING_DOM_ERROR, 0, 0, 0);
  errno = 0;
  check_float ("pow (0, -11.1) == inf plus division by zero exception",  FUNC(pow) (0, -11.1L), plus_infty, 0, 0, DIVIDE_BY_ZERO_EXCEPTION);
  check_int ("errno for pow(0,-num) == DIFFERING_DOM_ERROR", errno, DIFFERING_DOM_ERROR, 0, 0, 0);
  errno = 0;
...
sincos_test (void)
{
  ...
  check_float ("sincos (0.80190127184058835, &sin_res, &cos_res) puts 0.69534156199418473 in cos_res", cos_res, 0.69534156199418473, DELTA2650, 0, 0);
#endif
...
y0_test (void)
{
  ...
  check_float ("y0 (0.125) == -1.38968062514384052915582277745018693",  FUNC(y0) (0.125L), -1.38968062514384052915582277745018693L, DELTA2779, 0, 0);
...
yn_test (void)
{
  ...
  check_float ("yn (0, 0.125) == -1.38968062514384052915582277745018693",  FUNC(yn) (0, 0.125L), -1.38968062514384052915582277745018693L, DELTA2801, 0, 0);
  ...
  if (remaining != argc)
    {
      ...
      argp_help (&argp, stdout, ARGP_HELP_SEE, __progname);
      exit (EXIT_FAILURE);
    }
#endif
  if (output_ulps)
